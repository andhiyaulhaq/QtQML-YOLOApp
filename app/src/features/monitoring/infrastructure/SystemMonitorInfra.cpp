#include "SystemMonitorInfra.h"
#include <QCoreApplication>

SystemMonitorInfra::SystemMonitorInfra(QObject *parent)
    : QObject(parent)
    , m_timer(new QTimer(this))
{
    m_timer->setInterval(1000);
    connect(m_timer, &QTimer::timeout, this, &SystemMonitorInfra::updateMetrics);
    initializePlatform();
}

SystemMonitorInfra::~SystemMonitorInfra()
{
    cleanupPlatform();
}

void SystemMonitorInfra::startMonitoring()
{
    if (!m_timer->isActive()) {
        m_timer->start();
        updateMetrics();
...
void SystemMonitorInfra::stopMonitoring()
{
    if (m_timer->isActive()) {
        m_timer->stop();
    }
...
void SystemMonitorInfra::updateMetrics()
{
    SystemStats stats;
...
void SystemMonitorInfra::initializePlatform()
{
#ifdef Q_OS_WIN
...
void SystemMonitorInfra::cleanupPlatform()
{
#ifdef Q_OS_WIN
...
double SystemMonitorInfra::getCpuUsage()
{
#ifdef Q_OS_WIN
...
QString SystemMonitorInfra::getSystemMemoryInfo()
{
#ifdef Q_OS_WIN
...
QString SystemMonitorInfra::getProcessMemoryInfo()
{
#ifdef Q_OS_WIN
